...
class App(tk.Tk):
    ...
    def _buildBasicSection(self, parent):
        ...
        winInner.grid(row=winRow, column=1, columnspan=2, sticky="w", pady=2)
        ttk.Radiobutton(winInner, text="Windowed  (--windowed)",
                        variable=self.varConsole, value=False).pack(side="left", padx=(0, PAD))
        ttk.Radiobutton(winInner, text="Console   (--console)",
                        variable=self.varConsole, value=True).pack(side="left")
