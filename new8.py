...
class App(tk.Tk):
    ...
    def _buildBasicSection(self, parent):
        ...
        rows = [
            ...
            ("  Python Interpreter",self.varPython,   self._browsePython),
        ]
    ...
    def _buildNotebook(self, parent):
        ...
        # Row 0: Output Mode label + radio buttons
        ttk.Label(t1, text="Output Mode", width=14, anchor="w").grid(
            row=0, column=0, sticky="w", padx=(0, PAD), pady=2)
        ttk.Radiobutton(t1, text="One File  (--onefile)",
                        variable=self.varOnefile, value=True).grid(row=0, column=1, sticky="w", padx=(0, PAD))
        ttk.Radiobutton(t1, text="One Dir   (--onedir)",
                        variable=self.varOnefile, value=False).grid(row=0, column=2, sticky="w")

        # Row 1: Window Mode label + radio buttons
        ttk.Label(t1, text="Window Mode", width=14, anchor="w").grid(
            row=1, column=0, sticky="w", padx=(0, PAD), pady=2)
        ttk.Radiobutton(t1, text="Windowed  (--windowed)",
                        variable=self.varConsole, value=False).grid(row=1, column=1, sticky="w", padx=(0, PAD))
        ttk.Radiobutton(t1, text="Console   (--console)",
    ...
    def _buildPreview(self, parent):
        ...
        self.previewVar = tk.StringVar()
        previewEntry = ttk.Entry(row, textvariable=self.previewVar,
                                 state="readonly", font=("Consolas", 9))
        previewEntry.pack(side="left", fill="x", expand=True)

        ttk.Button(row, text="Copy", width=6, command=self._copyPreview).pack(side="left", padx=(4, 0))

    def _copyPreview(self):
        self.clipboard_clear()
        self.clipboard_append(self.previewVar.get())
    ...
    def _bindTrace(self):
        for var in (self.varScript, self.varName, self.varDistPath,
                    self.varIcon, self.varOnefile, self.varConsole, self.varPython):
            var.trace_add("write", lambda *_: self._updatePreview())

    def _buildCommand(self):
        python = self.varPython.get().strip()
        ...
        if script:
            # 優先用 _runBuild 已算好的值，否則即時計算供 Preview 顯示
            outputFolder = getattr(self, "_outputFolder", None)
            if not outputFolder:
                name     = self.varName.get().strip()
                distPath = os.path.abspath(self.varDistPath.get().strip() or "dist")
                outputFolder = os.path.join(distPath, f"{name}_Output") if name else distPath
    ...
    def _updatePreview(self):
        self.previewVar.set(" ".join(self._buildCommand()))
...
{'exe = EXE(' if True else ''}
    ...
    icon={iconStr},
    console={console},
    {'bootloader_ignore_signals=False,' if onefile else ''}
)
"""
        if not onefile:
            specContent += f"""
